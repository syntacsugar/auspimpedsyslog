...
RUN apt-get update \
    && apt-get install -y git net-tools vim nginx rsyslog supervisor php7.3-fpm php7.3-cli apache2-utils\
    && rm -rf /var/lib/apt/lists/*

RUN sed -i -e 's/listen\ =\ 127.0.0.1:9000/listen\ =\ \/var\/run\/php7.3-fpm.sock/' /etc/php7.3/fpm/pool.d/www.conf
RUN sed -i '1idaemon off;' /etc/nginx/nginx.conf

RUN rm -rf /var/www && git clone https://github.com/potsky/PimpMyLog.git /var/www
RUN sed -i -e 's/;daemonize\ =\ yes/daemonize\ =\ no/' /etc/php7.3/fpm/php-fpm.conf
RUN sed -i 's/^variables_order\ =.*/variables_order\ =\ \"GPCSE\"'/ /etc/php7.3/cli/php.ini
...
#RUN cd /var/www && php7.3 -f ./create-user.php && chown www-data:www-data config.auth.user.php 
